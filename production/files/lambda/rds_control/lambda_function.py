...
def lambda_handler(event, context):
    ...
    for rds_clusters in rds_clusters:
        try:
            ...
            logger.info("DBClusterIdentifier is %s and Status is %s" % (cluster_ids, cluster_status))
            ...
            for tags in tags:
                if tags['Key'] == 'AutoStop':
                    logger.info("Current instance_state of %s is %s" % (cluster_ids, cluster_status))

                    if action == 'stop':
                        client.stop_db_cluster(DBClusterIdentifier=cluster_ids)
                        logger.info("Aurora Cluster %s comes to stop" % cluster_ids)
                    elif action == 'start':
                        client.start_db_cluster(DBClusterIdentifier=cluster_ids)
                        logger.info("Aurora Cluster %s comes to start" % cluster_ids)
                    else:
                        logger.info("Aurora Cluster %s status is not right to start" % cluster_ids)
